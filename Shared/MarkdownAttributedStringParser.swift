...
/// Based on Markdownosaur by Christian Selig
/// Link: https://github.com/christianselig/Markdownosaur
struct MarkdownAttributedStringParser: MarkupVisitor {
    ...
    let newlineSize: CGFloat = 10
    
    #if os(iOS) || os(macOS) || os(tvOS)
    let highlighter: Highlighter = {
        let highlighter = Highlighter()!
        highlighter.setTheme(HighlighterConstants.theme)
        return highlighter
    }()
    #endif
    ...
    public mutating func parserResults(from document: Document) -> [ParserResult] {
            ...
            if let codeBlock = markup as? CodeBlock {
                if !currentAttrString.string.isEmpty {
                    ...
                    results.append(ParserResult(
                        attributedString: currentAttrStringToAppend, isCodeBlock: false,
                        codeBlockLanguage: nil))
                }
                
                #if os(macOS)
                let attrStringToAppend = (try? AttributedString(attrString, including: \.appKit)) ?? AttributedString(stringLiteral: currentAttrString.string)
                #else
                let attrStringToAppend = (try? AttributedString(attrString, including: \.uiKit)) ?? AttributedString(stringLiteral: currentAttrString.string)
                #endif
                
                results.append(ParserResult(attributedString: attrStringToAppend, isCodeBlock: true, codeBlockLanguage: codeBlock.language))
    ...
    mutating public func visitParagraph(_ paragraph: Paragraph) -> NSAttributedString {
        ...
        if paragraph.hasSuccessor {
            result.append(paragraph.isContainedInList ? .singleNewline(withFontSize: newlineSize) : .doubleNewline(withFontSize: newlineSize))
        }
    ...
    mutating public func visitHeading(_ heading: Heading) -> NSAttributedString {
        ...
        if heading.hasSuccessor {
            result.append(.doubleNewline(withFontSize: newlineSize))
        }
    ...
    public func visitCodeBlock(_ codeBlock: CodeBlock) -> NSAttributedString {
        ...
        #else
        let result = NSMutableAttributedString(string: codeBlock.code, attributes: [.font: UIFont.monospacedSystemFont(ofSize: baseFontSize - 1.0, weight: .regular), .foregroundColor: UIColor.white])
        #endif
        
        #if os(tvOS)
        result.addAttributes([.font: UIFont.monospacedSystemFont(ofSize: baseFontSize - 1.0, weight: .regular)])
        #endif
        
        if codeBlock.hasSuccessor {
            result.append(.singleNewline(withFontSize: newlineSize))
        }
    ...
    mutating public func visitOrderedList(_ orderedList: OrderedList) -> NSAttributedString {
        ...
        if orderedList.hasSuccessor {
            result.append(orderedList.isContainedInList ? .singleNewline(withFontSize: newlineSize) : .doubleNewline(withFontSize: newlineSize))
        }
...
extension UIColor {
    ...
    static let systemPink = UIColor.magenta
    static let systemBlue = UIColor.blue
}
